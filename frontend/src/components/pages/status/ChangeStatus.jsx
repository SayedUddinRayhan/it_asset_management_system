...
function ChangeStatus() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Preselect product if coming from products page
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.patch(
        `${API}/products/${form.product.value}/`,
        { status: form.status.value }
      );
      toast.success("Product status updated successfully");

      // Update local allProducts table
      setAllProducts((prev) =>
        prev.map((p) =>
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-white shadow-xl rounded-2xl p-6">
            ...
            <div className="md:col-span-2 flex justify-end gap-3 pt-4">
                ...
                className="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-lg text-sm font-medium shadow-sm"
                >
                Cancel
                </button>
                ...
                type="submit"
                disabled={saving}
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-lg text-sm font-medium shadow-sm flex items-center gap-2"
                >
        ...
        <div className="bg-white shadow-xl rounded-2xl p-6 overflow-x-auto">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">All Products Status</h3>
